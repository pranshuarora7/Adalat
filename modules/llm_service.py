...
genai.configure(api_key=os.getenv("GEMINI_API_KEY") or "AIzaSyAVL1hfzDmgoQNeN-MaE6_mKJM_56vRhgQ")
...
model = genai.GenerativeModel(
    ...
    system_instruction=(
        ...
        "You provide helpful, detailed, and accurate responses only about law, legal cases, legal processes, and how to proceed legally. "
        "You do not answer questions outside law. Speak formally and professionally, as a real lawyer would. "
        "Always analyze the case, summarize it, give a legal roadmap, and suggest next steps when applicable."
    )
)

def summarize_case(text):
    prompt = f"Summarize the following legal case in plain language:\n\n{text}"
    return model.generate_content(prompt).text

def generate_roadmap(text):
    prompt = f"Given this legal case, provide a roadmap outlining the key legal steps, stakeholders, and potential outcomes:\n\n{text}"
    return model.generate_content(prompt).text

def suggest_next_steps(text):
    prompt = f"What are the next legal actions or steps that can be taken in the following case?\n\n{text}"
    return model.generate_content(prompt).text

def classify_query(text):
    """Heuristic classification for roadmap/summary/next."""
    text_lower = text.lower()
    if any(word in text_lower for word in ["next step", "should i do", "what now", "next action"]):
        return "next"
    elif any(word in text_lower for word in ["summary", "summarize", "explain"]):
        return "summary"
    elif any(word in text_lower for word in ["roadmap", "legal process", "procedure", "stages"]):
        return "roadmap"
    elif len(text.split()) > 50:  # likely a full case description
        return "full_case"
    else:
        return "general"
...
def handle_query(user_input):
    ...
    if query_type == "roadmap":
        ...
        summary = summarize_case(user_input)
        return f"📘 Summary:\n{summary}"

    elif query_type == "next":
        next_steps = suggest_next_steps(user_input)
        return f"➡️ Next Steps:\n{next_steps}"
        ...
        # fallback to smart general chat
        return model.generate_content(user_input).text

# ✅ Main chatbot loop
if __name__ == "__main__":
    print("🧑‍⚖️ Welcome to Adalat Legal Assistant")
    print("Ask your legal question or paste a case description. Type 'exit' to quit.\n")
